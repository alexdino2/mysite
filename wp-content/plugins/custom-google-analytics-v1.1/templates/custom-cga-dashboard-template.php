...
if(!empty($gaAccounts)){
	...
	if(isset($_GET['account']) && $_GET['account'] != ''){
		$days = 30;
                if(!isset($startDate))
                    {   
                        ...
                        {
                            $startDate = date("m/d/Y", strtotime($_SESSION['startDate']));

                        }   
                        else
                        {
                            $startDate = date("m/d/Y", time() - 3000000);
                        }
                    }
                if(!isset($endDate))
                {   
                    ...
                    {
                        $endDate = date("m/d/Y", strtotime($_SESSION['endDate']));

                    }   
                    else
                    {
                        $endDate = date("m/d/Y", time() - 86400);
                    }
                }
		...
		if(!empty($gaPropereties)){?>
			<div class="filters">
				<span><?php if($_GET['startDate'] != '' && $_GET['endDate'] != ''){
