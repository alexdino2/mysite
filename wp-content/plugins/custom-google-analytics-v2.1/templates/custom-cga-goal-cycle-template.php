...
If(isset($_SESSION['CGA_sessionData'])){
	...
	if(!$gAuth->isAccessTokenExpired()){
		...
		wp_redirect(get_bloginfo('url').'/google-analytics-access');
	}
}else{
	wp_redirect(get_bloginfo('url').'/google-analytics-access');
}
			...
			<table id="gaSiteGoalData" class="display dataTable no-footer" cellspacing="0" width="100%">
				...
				?>
					<tr role="row" class="odd">
						<td class="sorting_1"><a href="<?php echo $siteMetaPageURL.'?account='.$_GET['account'].'&property='.$item['id'].'&propertyname='.$item['name'].'&days='.$days.'&profileId='.$item['profileID'] ?>"><?php echo $item['name'].' - '.$item['id']; ?></a></td>
					...
					?>
					<tr>
						<td ><a href="<?php echo $siteMetaPageURL.'?account='.$_GET['account'].'&property='.$item['id'].'&propertyname='.$item['name'].'&days='.$days.'&profileId='.$item['profileID'].'&goalID='.$id ?>"> <?php echo $data['name'].'- goal '.$j; ?></a></td>
						...
						<td > <?php echo $item['metaResult']['goalConversionRate']; ?></td>
				...
				?>
					<tr role="row" class="odd">
						...
						<td>&nbsp;<?php //echo $item['metaData']['sessions']; ?></td>
						<td>
							<a href="<?php echo $siteMetaPageURL.'?account='.$_GET['account'].'&property='.$item['id'].'&propertyname='.$item['name'].'&days='.$days.'&profileId='.$item['profileID'].'&allGoalComplete='.$totalGoalCompletion.'&session='.$session ?>">
