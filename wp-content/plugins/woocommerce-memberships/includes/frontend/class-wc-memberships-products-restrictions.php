...
class WC_Memberships_Products_Restrictions {
	...
	public function product_is_purchasable( $purchasable, $product ) {
		...
		// if this method is not being used as a callback and the product is a variation, defer to specific method for variations
		if ( 'woocommerce_is_purchasable' !== current_filter() && $product->is_type( 'variation' ) ) {

			$purchasable = $this->variation_is_purchasable( $purchasable, $product );

		// standard behavior as direct method or filter callback for all other products
		} else {
