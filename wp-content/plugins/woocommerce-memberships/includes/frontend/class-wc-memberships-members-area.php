...
class WC_Memberships_Members_Area {
	...
	private function get_members_area_memberships_endpoint() {
		...
		if ( 1 === count( $user_memberships ) && ( $user_membership = $user_memberships[0] ) && $user_membership->is_active() ) {
			...
			$url = ! $this->using_permalinks && SV_WC_Helper::str_starts_with( $url, '&' ) ? ltrim( $url, '&' ) : untrailingslashit( $url );
		} else {
			$url = $this->endpoint;
		}
		...
		if ( $this->is_members_area_section() ) {
			...
			if ( ! empty( $members_area_endpoint ) && wc_memberships()->get_plans_instance()->get_membership_plans_count() > 0 ) {
				...
				if ( array_key_exists( 'orders', $items ) ) {
					$items = SV_WC_Helper::array_insert_after( $items, 'orders', array( $endpoint => $endpoint_title ) );
				} else {
	...
	private function get_members_area_user_membership_details( $user_membership ) {
		...
		$details = array(
			'status' => array(
				...
				'content' => ucwords( $user_membership->get_status() ),
	...
	public function __call( $method, $args ) {
		...
		switch ( $method ) {
			...
			/* @deprecated between 1.7.4 and 1.9.0 - remove by version 1.11.0 or higher {FN 2017-08-18} */
			case 'render_member_area_content' :
			case 'render_members_area_content' :
				$deprecated_since = 'render_member_area_content' === $method ? '1.7.4' : '1.9.0';
				_deprecated_function( $deprecated, $deprecated_since, "{$class}::output_members_area()" );
				$this->output_members_area();
				return null;
