...
/**
 * Returns a members area URL.
 *
 * @since 1.4.0
 *
 * @param int|\WC_Memberships_Membership_Plan $membership_plan plan object or id
...
function wc_memberships_get_members_area_url( $membership_plan, $members_area_section = '', $paged = '' ) {
	...
	if ( ! $my_account_page_id > 0 || ! $membership_plan_id || 0 === $membership_plan_id ) {
		return '';
	}
...
function wc_memberships_get_members_area_action_links( $section, $user_membership, $object = null ) {
	...
	switch ( $section ) {
		...
		case 'my-membership-details' :
			...
			if (    'my-membership-details' !== $section
				...
				// perhaps open the details section, if available, or default to the first section in array
				if ( in_array( 'my-membership-details', $sections, true ) ) {
					$url_section = 'my-membership-details';
				} else {
	...
	// TODO throw a deprecation notice for now, later remove the deprecated filter by version 1.11.0 {FN 2017-03-20}
...
function wc_memberships_get_members_area_sorting_link( $sort_key, $sort_label ) {
	...
	if ( $members_area = wc_memberships()->get_frontend_instance()->get_members_area_instance() ) {
		...
		if ( empty( $sorting_args ) ) {
			...
			if ( 'title' === $sort_key ) {
				$sorting_link .= '<a class="sort-by-post-title" href="' . esc_url( $sort_url ) . '">' . esc_html( $sort_label );
			} elseif ( 'type' === $sort_key ) {
				$sorting_link .= '<a class="sort-by-post-type" href="' . esc_url( $sort_url ) . '">' . esc_html( $sort_label );
			}
