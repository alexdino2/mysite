<?php class Google_Analytics {
	...
	public function getOppsv3($accountID,$profileID, $days, $goalID = 0) {
		...
		//$arr = array('ga:medium', 'ga:deviceCategory','ga:browser','ga:operatingSystem','ga:region', 'ga:campaign','ga:browserSize','ga:interestInMarketCategory','ga:interestAffinityCategory','ga:keyword');
                $arr = array('ga:medium', 'ga:deviceCategory','ga:campaign','ga:interestInMarketCategory','ga:interestAffinityCategory','ga:keyword','ga:browser','ga:dataSource','ga:landingPagePath');
            ...
            foreach($array as $whichdim) {
                ...
                foreach ($whichdim['data']->getRows() as $row) {
                    if ($row[1]>1) { //only keep if den is greater than 30
                        ...
                        $newarray[]= array('whichdim'=>$whichdim['whichdim'],'dim'=>$row[0],'den'=>$row[1],'num'=>$row[2], 'opps'=>number_format((float)$this->calcOppsLoc($row[2],$row[1],$whichdim['data']['totalsForAllResults']['ga:goal1Completions'],$mem->get('sessions')), 2, '.', ''),'id'=>$whichdim['data']['id'],'quer'=>$_SERVER['REQUEST_URI'].'&filter='.$whichdim['whichdim'].'=='.$row[0].'||drill down');
