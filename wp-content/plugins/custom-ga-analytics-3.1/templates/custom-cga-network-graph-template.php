...
if(!empty($gaAccounts)){
	if(isset($_GET['account']) && $_GET['account'] != '' && isset($_GET['property']) && $_GET['property'] != '' ){
		...
		if(isset($_GET['goalID'])){
			...
			$gaPropereties = $gAnalytics->getListGoalsMeta($_GET['account'],$_GET['profileId'], $days, $_GET['goalID']);
                        ...
                        if(isset($_GET['filter'])){
                            $gaNetwork = $gAnalytics->getNetworkData($_GET['account'],$_GET['profileId'], date('Y-m-d', strtotime($_GET['startDate'])), date('Y-m-d', strtotime($_GET['endDate'])), $_GET['goalID'], $_GET['filter']);
                            //$mem->set($user_id.'networkdata',json_encode($gaNetwork));
                            $mem->set('networkdata',json_encode($gaNetwork));
                            
                            $gaPageStats = $gAnalytics->getPageStats($_GET['account'],$_GET['profileId'], date('Y-m-d', strtotime($_GET['startDate'])), date('Y-m-d', strtotime($_GET['endDate'])), $_GET['goalID'], $_GET['filter']);
                            $mem->set($user_id.'pagestats',json_encode($gaPageStats));
                        }
                        else{
                            $gaNetwork = $gAnalytics->getNetworkData($_GET['account'],$_GET['profileId'], date('Y-m-d', strtotime($_GET['startDate'])), date('Y-m-d', strtotime($_GET['endDate'])), $_GET['goalID'], NULL);
                            //$mem->set($user_id.'networkdata',json_encode($gaNetwork));
                            $mem->set('networkdata',json_encode($gaNetwork));
                            
                            $gaPageStats = $gAnalytics->getPageStats($_GET['account'],$_GET['profileId'], date('Y-m-d', strtotime($_GET['startDate'])), date('Y-m-d', strtotime($_GET['endDate'])), $_GET['goalID'], NULL);
                            $mem->set($user_id.'pagestats',json_encode($gaPageStats));
                        }
			...
			$gaPropereties = $gAnalytics->getListGoalsMeta($_GET['account'],$_GET['profileId'], $days);
                        //$gaOpportunities = $gAnalytics->getNetworkData($_GET['account'],$_GET['profileId'], date('Y-m-d', strtotime($_GET['startDate'])), date('Y-m-d', strtotime($_GET['endDate'])), $_GET['goalID'], NULL);
                        $gaNetwork = $gAnalytics->getNetworkData($_GET['account'],$_GET['profileId'], date('Y-m-d', strtotime($_GET['startDate'])), date('Y-m-d', strtotime($_GET['endDate'])), $_GET['goalID'], NULL);
                        $mem->set($user_id.'networkdata',json_encode($gaNetwork));                        
		...
		if(isset($_GET['goalID']) || isset($_GET['allGoalComplete'])){
			//echo '<div class="goalMetaData">';
				echo "Site Name: ".$gaPropereties['siteName']."<br/>";
                                ...
                                echo "Site Optimus has constructed your network and compiled your page stats.<br/>";
                                echo '<audio autoplay>';
                                echo '<source src="https://siteoptimus.com/Elasticacut.mp3" type="audio/mpeg">';
                                echo '</audio>';
