<?php class Google_Analytics {
    ...
    public function getListGoals($accountID, $startdate, $enddate) {
		$properties = $this->analytics->management_webproperties->listManagementWebproperties($accountID);
		...
		if (count($properties->getItems()) > 0) {
			...
			foreach($items as $item){
				...
				$metadata = $this->getSiteMetadataNew($accountID, $item->id, $startdate, $enddate);
				...
				foreach($properties1 as $data){
					...
					$id = $data['id'];
					$funnelMeta[] = $this->analytics->data_ga->get($profileId = 'ga:'.$metadata['modelData']['profileInfo']['profileId'], $startDate =$startdate, $endDate = $enddate, $metrics = 'ga:sessions,ga:pageviews,ga:goal'.$id.'Completions'); 
				}
	...
	public function getListGoalsMeta($accountID,$profileID, $days, $goalID = 0) {
        ...
        public function getListGoalsMetaNew($accountID,$profileID, $startdate,$enddate, $goalID = 0) {
		$properties = $this->analytics->management_webproperties->listManagementWebproperties($accountID);
		...
		if($goalID > 0){
			...
			$funnelMeta = array();
			$funnelMeta = $this->analytics->data_ga->get($profileId = 'ga:'.$profileID, $startDate =$startdate, $endDate = $enddate, $metrics = 'ga:sessions,ga:pageviews,ga:goal'.$goalID.'Completions');
			$realtime['goalMeta'] = $funnelMeta;
		}
	...
	public function getOppsv3($accountID,$profileID, $days, $goalID = 0) {
        ...
        public function getOppsCyclev31($accountID,$profileID, $startdate, $enddate, $goalID = 0, $filter = NULL) {
		//$properties = $this->analytics->management_webproperties->listManagementWebproperties($accountID);

		//$optParams = array('dimensions' => array('rt:operatingSystem','rt:medium','rt:deviceCategory','rt:mobileDeviceModel', 'rt:campaign', 'rt:browser','rt:region'));
		//$arr = array('ga:medium', 'ga:deviceCategory','ga:browser','ga:operatingSystem','ga:region', 'ga:campaign','ga:browserSize','ga:interestInMarketCategory','ga:interestAffinityCategory','ga:keyword');
                $arr = array('ga:medium', 'ga:deviceCategory','ga:campaign','ga:interestInMarketCategory','ga:interestAffinityCategory','ga:userGender','ga:userAgeBracket','ga:keyword','ga:browser','ga:dataSource','ga:landingPagePath','ga:adContent','ga:language','ga:flashVersion','ga:javaEnabled','ga:screenResolution','ga:searchUsed','ga:searchKeyword','ga:country','ga:region','ga:city','ga:dayOfWeek','ga:dateHour','ga:userType');
		...
		//$optParams = array('dimensions' => 'rt:medium','dimensions' => 'rt:deviceCategory','dimensions' => 'rt:mobileDeviceModel','dimensions' => 'rt:campaign','dimensions' => 'rt:browser','dimensions' => 'rt:region','dimensions' => 'rt:operatingSystem');
                ...
                foreach ($arr as $whichdim1) {
                    ...
                    //$ga = $this->analytics->data_ga->get($profileId = 'ga:'.$profileID, $startDate = $startdate, $endDate = $enddate, $metrics = 'ga:sessions,ga:goal'.$goalID.'Completions',$optParams);
                    $ga = $this->analytics->data_ga->get($profileId = 'ga:'.$profileID, $startDate = $startdate, $endDate = $enddate, $metrics = 'ga:sessions,ga:pageViews',$optParams);
                    //$ga = $this->analytics->data_ga->get($profileId = 'ga:'.$profileID, $startDate = $startdate, $endDate = $enddate, $metrics = 'ga:sessions,ga:bounces',$optParams);
                    //$ga['siteName'] = $properties['modelData']['items'][0]['name'];
        ...
        ///copied from getOppsCycle31
        public function getNetworkData($accountID,$profileID, $startdate, $enddate, $goalID = 0, $filter = NULL) {
		//$properties = $this->analytics->management_webproperties->listManagementWebproperties($accountID);

		//$arr = array('ga:medium', 'ga:deviceCategory','ga:browser','ga:operatingSystem','ga:region', 'ga:campaign','ga:browserSize','ga:interestInMarketCategory','ga:interestAffinityCategory','ga:keyword');
                //$arr = array('ga:previousPagePath', 'ga:pagePath');

                $optParams = array('dimensions' =>'ga:previousPagePath,ga:pagePath','filters'=>'ga:previousPagePath!=(entrance);ga:pageViews>0');
         
                
                $ga = $this->analytics->data_ga->get($profileId = 'ga:'.$profileID, $startDate = $startdate, $endDate = $enddate, $metrics = 'ga:uniquePageviews,ga:pageViews',$optParams);

                //$results[] = array("whichdim"=>'network',"data"=>$ga);
                $results[] = $ga;
        ...
        ///copied from getNetworkData
        public function getPageStats($accountID,$profileID, $startdate, $enddate, $goalID = 0, $filter = NULL) {
		//$properties = $this->analytics->management_webproperties->listManagementWebproperties($accountID);

		//$arr = array('ga:medium', 'ga:deviceCategory','ga:browser','ga:operatingSystem','ga:region', 'ga:campaign','ga:browserSize','ga:interestInMarketCategory','ga:interestAffinityCategory','ga:keyword');
                //$arr = array('ga:previousPagePath', 'ga:pagePath');

                $optParams = array('dimensions' =>'ga:pagePath','filters'=>'ga:pageViews>0');
         
                
                $ga = $this->analytics->data_ga->get($profileId = 'ga:'.$profileID, $startDate = $startdate, $endDate = $enddate, $metrics = 'ga:uniquePageviews,ga:pageViews,ga:exits,ga:entrances,ga:pageLoadTime,ga:avgPageLoadTime',$optParams);
...
*/		
    ...
    public function getSiteMetadataNew($accountID, $propertyID, $startdate, $enddate) {
		$profiles = $this->analytics->management_profiles->listManagementProfiles($accountID, $propertyID);
		$results = array();
		if (count($profiles->getItems()) > 0) {
			$items = $profiles->getItems();
			//$results = $this->getResults($this->analytics, $items[0]->getId(), $days);
                        $results = $this->getResultsNew($this->analytics, $items[0]->getId(), $startdate, $enddate);
			return $results;
		} else {
			return $results;
		}
	...
	function getResults(&$analytics, $profileId, $days) {
	  ...
	  // for the last seven days.
	   return $analytics->data_ga->get(
        ...
        function getResultsNew(&$analytics, $profileId, $startdate, $enddate) {
	  // Calls the Core Reporting API and queries for the number of sessions
	  // for the last seven days.
	   return $analytics->data_ga->get(
		   'ga:' . $profileId,
	...
	function saveCycleTable(&$res) {
        ...
        function array_flatten($array,$id) {
            $newarray = array();
            ...
            $mem = new Memcached();
                            ...
                            $numsum=$mem->get($user_id.'goalcompletes');
                            $densum=$mem->get($user_id.'sessions');
            
            foreach($array as $whichdim) {
                
                if($whichdim['data']->getRows()){
                    foreach ($whichdim['data']->getRows() as $row) {
                        
                        if ($row[1]>=1) { //only keep if den is greater than 30
                            //need to append other parametercs to support hyperlinking
                            //$newarray[]= array('whichdim'=>$whichdim['whichdim'],'dim'=>$row[0],'den'=>$row[1],'num'=>$row[2], 'opps'=>number_format((float)$this->calcOppsLoc($row[2],$row[1],$whichdim['data']['totalsForAllResults']['ga:goal1Completions'],$whichdim['data']->totalsForAllResults['ga:sessions']), 2, '.', ''),'id'=>$whichdim['data']['id'],'quer'=>$_SERVER['REQUEST_URI'].'&filter='.$whichdim['whichdim'].'=='.$row[0].'||drill down');

                            //$opps=number_format((float)$this->calcOppsLoc($row[2],$row[1],$mem->get('goalcompletes')-$row[2],$mem->get('sessions')-$row[1]), 2, '.', '');
                            $opps=number_format((float)$this->calcOppsLoc($row[2],$row[1],$numsum-$row[2],$densum-$row[1]), 2, '.', '');
                            if ($opps>.4*$numsum) 
                                {
                                    $opplevel='High';
                                }
                            elseif ($opps>.15*$numsum)
                                {
                                    $opplevel='Med';
                                }
                            else
                            ...
                            //$newarray[]= array('whichdim'=>$whichdim['whichdim'],'dim'=>$row[0],'den'=>$row[1],'num'=>$row[2], 'opps'=>$opps,'id'=>$whichdim['data']['id'],'opplevel'=>$opplevel,'quer'=>$_SERVER['REQUEST_URI'].'&filter='.$whichdim['whichdim'].'=='.$row[0].'||drill down');
                            $desc="There were ".$row[1]." sessions where ".$whichdim['whichdim']." was ".$row[0]." with ".$row[2]." successes (rate of ".$row[2]/$row[1]."). We estimate about ".$opps." opportunities were missed.";
                            $newarray[]= array('whichdim'=>$whichdim['whichdim'],'dim'=>$_SERVER['REQUEST_URI'].'&filter='.$whichdim['whichdim'].'=='.$row[0].'||'.$row[0],'den'=>$row[1],'num'=>$row[2], 'opps'=>$opps,'id'=>$whichdim['data']['id'],'opplevel'=>$opplevel,'quer'=>$_SERVER['REQUEST_URI'].'&filter='.$whichdim['whichdim'].'=='.$row[0].'||drill down','desc'=>$desc);
                            //$newarray[]= array('whichdim'=>$whichdim['whichdim'],'dim'=>$row[0],'den'=>$row[1],'num'=>$row[2], 'opps'=>number_format((float)$this->calcOppsLoc($row[2],$row[1],$mem->get('goalcompletes')-$row[2],$mem->get('sessions')-$row[1]), 2, '.', ''),'id'=>$whichdim['data']['id'],'quer'=>$_SERVER['REQUEST_URI'].'&filter='.$whichdim['whichdim'].'=='.$row[0].'||drill down');

                            //$numofentries=count($newarray);
                ...
                $mem->set($user_id.'dimdennum',json_encode($newarray));
	...
	function calculateTime ($seconds){
        ...
        function calcOppsLoc ($num,$den,$antinumsum,$antidensum) {
                
            //$response = round(($antinumsum/$antidensum - $num/$den) * $den,0);
            if ($antidensum==0){
                $response = 0;
            }
            else
            {
                $response = round(($antinumsum/$antidensum) * $den,0);
            }
            $response = json_decode($response);
            return($response);
        }
        
        function saveKPI(&$res) {//needs updating
            if (count($res) > 0) {
                //$results = array();
                //$results = $res['data']->getRows();
                //$columns = implode(", ",array_keys($res['data']->getColumnHeaders()));
                $columns = 'kpiid,kpiname,userid,siteid,goalid';
        ...
        //from GAPI
        public function requestReportData($profileId, $dimensions, $metrics, $sort_metric=null, $filter=null, $start_date=null, $end_date=null, $start_index=1, $max_results=1000)
        {
