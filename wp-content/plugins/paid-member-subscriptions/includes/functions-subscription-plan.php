    ...
    function pms_output_subscription_plans( $include = array(), $exclude_id_group = array(), $member = false, $default_checked = '' ) {
        ...
        if( !empty( $subscription_plan_groups ) ) {
            ...
            if( !$member && count( $subscription_plan_groups ) == 1 && count( $subscription_plan_groups[ key($subscription_plan_groups) ] ) == 1 ) {
                ...
                $output .= '<input type="hidden" name="subscription_plans" value="' . esc_attr( $subscription_plan_groups[ key($subscription_plan_groups) ][0]->id ) . '" />';;
                ...
                foreach( $subscription_plan_groups as $top_parent_id => $subscription_plans ) {
                    ...
                    if( !$member ) {
                        ...
                        foreach( $subscription_plans as $subscription_plan ) {
                            ...
                            $subscription_plan_output .= $subscription_plan->name . '</label>';
                            ...
                            if( $subscription_plan->price == 0 )
                                ...
                                $price_output = '<span class="pms-subscription-plan-price">' . $subscription_plan->price . '</span>' . ' ' . '<span class="pms-subscription-plan-currency">' . $currency_symbol . '</span>';
                            }

                            $subscription_plan_output .= apply_filters( 'pms_subscription_plan_output_price', '<span class="pms-divider"> - </span>' . $price_output, $subscription_plan );
                            ...
                            $subscription_plan_output .= apply_filters( 'pms_subscription_plan_output_duration', $duration_output, $subscription_plan );
                        ...
                        foreach( $subscription_plans as $subscription_plan ) {
                            ...
                            if( $expiration_time_stamp > time() + 60 * 60 * 24 * 365 * 5 )
                                $expiration_date_output = 'Unlimited';
                            else
                            ...
                            if( $member_subscription['status'] == 'expired' )
                                $expiration_date_output = __( 'Expired on: ', 'paid-member-subscriptions' ) . $expiration_date_output;
