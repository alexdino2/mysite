		...
		if(!empty($gaPropereties)){?>
			...
			<table id="gaSiteMetaData" class="display" cellspacing="0" width="100%">
				...
				<?php foreach($gaPropereties as $item){ ?>
					<tr>
						<td><a href="<?php echo $goalCyclePageURL.'?account='.$_GET['account'].'&property='.$item['id'].'&propertyname='.$item['name'].'&days='.$days //changed url from $siteMetaPageURL ?>"><?php echo $item['name'].' - '.$item['id'] ?></a></td>
