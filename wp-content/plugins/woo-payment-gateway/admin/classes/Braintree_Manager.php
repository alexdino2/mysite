...
class Braintree_Manager{
	...
	public function activateLicense($license_key){
		...
		$url_args = array('slm_action'=>'slm_activate',
	...
	/**
	 * Generate a client token using the Braintree SDK. If a user has a customerId saved in the database, 
	...
	public function getClientToken($user_id = false){
		...
		try{
			...
			if($customerId){
				delete_user_meta($user_id, 'braintree_vault_id');
				$clientToken = $this->getClientToken();
			}
		}
		catch(Exception $e){
	...
	/**
	 * Perform the Braintree_Transaction::sale() for the given $attribs and $order. If the sale is successfull, the transaction data is 
	...
	public function woocommerceSale($attribs = array(), WC_Order $order){
		...
		try{
			...
			if($response->success){
				...
				$this->log->writeToLog(sprintf('Transaction %s successfully charged in the amount of %s for orderId %s',
						$response->transaction->id, $order->get_total(), $order->id));
	 ...
	 * @param Braintree_Transaction | Braintree_PaymentMethod $object
	 */
	...
	public function getPaymentMethodToken(Braintree_Transaction $transaction){
		...
		elseif($transaction->paymentInstrumentType === 'paypal_account'){
			$token = 'PayPal - '.$transaction->paypalDetails->token;
		}
		return $token;
	}
	...
	public function createBraintreeCustomer($userId = 0, $attribs){
		try{
			...
			if($response->success){
				update_user_meta($userId, 'braintree_vault_id', $response->customer->id);
	...
	public function updateBraintreeCustomer($user_id, $attribs = array()){
		...
		try{
			...
			else $this->log->writeErrorToLog($e->getMessage());
		}
	}
	
	public function getBraintreeCustomerId($userId){
		return get_user_meta($userId, 'braintree_vault_id', true);
	}
	
	/**
	 * Process the WooCommerce Subscription using the order_id provided. The order may contain multiple subscriptions and each
	...
	/**
	 * Process the Braintree subscription using the order_id provided. The order may contain several 
	 ...
	 * @param array $attribs
	 */
	public function createBraintreeSubscription(WC_Subscription $subscription, $attribs = array()){
	 ...
	 * @param WC_Subscription $subscription
	 * @param bool $update_status default is false. 
	 */
	...
	/**
	 * Fetch the planId for the given product.
	 * @param WC_Subscription $subscription
	 */
	public function getSubscriptionPlanId(WC_Subscription $subscription){
		$planId = null;
	...
	/**
	 * Save the transaction data for the subscription to the database. The transcation data is saved to the subscription
	...
	/**
	 * Returns an array o consiting of a result and redirect index. This method is to be used only for 
	...
	public function getEnvironment(){
		...
		if($this->get_option('license_status') === 'inactive'){
			$environment = 'sandbox';
		}
		return $environment;
	}
	...
	/**
	 * Method that gets all of the confiured plan Id's in the Braintree environment. 
	 */
	...
	public function getDropinContainer(){
		...
		<?php }?>
		<?php 
			$paymentMethods = $this->get_option('payment_methods');
			if(! empty($paymentMethods)){
				?>
				<div class="accepted-payment-methods">
				  <?php foreach($paymentMethods as $key=>$method){
				  		if(! empty(BT_Manager()->settings['payment_methods'][$key])){?>
				  			<div class="payment-method"><img src="<?php echo Braintree_PaymentMethods::paymentMethods()[$key]['src']?>"/></div>
				  <?php }
				  }?>
				</div>
				<?php
			}?>
		<div id="dropin-container" <?php echo $this->customerHasPaymentMethods() ? 'style="display: none"' : ''?>>
		  <?php if($this->customerHasPaymentMethods()){?>
		  	<div class="payment-method-button"><span id="cancel_add_new"><?php echo __('Cancel', 'braintree')?></span></div>
		  <?php }?>
		</div>
		<?php if(wp_get_current_user()->ID){?>
		<label <?php echo $this->customerHasPaymentMethods() ? 'style="display: none"' : ''?> class="save-payment-method-label"><span class="save-cc-helper"><?php echo __('Save', 'braintree')?></span><div class="save-payment-method"><input type="checkbox" id="save_payment_method" name="save_payment_method"><label for="save_payment_method"></label></div></label>
	<?php }
	}
	...
	public function getDonationDropinContainer(){
		if($this->getEnvironment() === 'sandbox'){?>
				<div class="gateway-environment"><?php echo __('Sandbox Mode', 'braintree')?></div>
				<?php 
					$paymentMethods = $this->get_option('donation_payment_methods');
					if(! empty($paymentMethods)){
						?>
						<div class="accepted-payment-methods">
						  <?php foreach($paymentMethods as $key=>$method){
	...
	public function loadCustomerPaymentMethods(){
		...
		if($customerId = BT_Manager()->getBraintreeCustomerId($user_id)){
			$customer = BT_Manager()->getBraintreeCustomer($customerId);
...
/**
 * Function that returns an instance of the Braintree_Manager class. If there is no instance, then a new instance is
