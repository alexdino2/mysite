...
class Braintree_Webhooks{
	...
	public static function braintreeHookAction(){
		if(isset($_REQUEST['woocommerce_braintree_subscription_hook'])){
			if(BT_Manager()->isActive('enable_webhooks')){
				...
				BT_Manager()->log->writeToLog((__('Webooks have not been enabled in the configuration. 
						Please enable webooks and test again.', 'braintree')));
			}
		}
	}
	...
	public static function processSubscriptionHook(){
		...
		try{
			...
			BT_Manager()->log->writeToLog(sprintf('There was an error parsing the webhook payload send 
	...
	public static function subscriptionChargedSuccessfully(WebhookNotification $notification){
		...
		try{
			if(BT_Manager()->isProductBraintreeOnlySubscription($subscription->id)){
			...
			BT_Manager()->log->writeToLog($e->getMessage());
		}
		BT_Manager()->log->writeToLog(sprintf('Subscription %s was charged successfully. Transcation ID: %s', $subscription->id, $bt_sub->transactions[0]->id));
	}
	...
	public static function subscriptChargedUnsuccessfully(WebhookNotification $notification){
		...
		try{
			...
			BT_Manager()->log->writeToLog(sprintf('Payment failed for subscription %s.', $subscription->id));
		}catch(Exception $e){
			BT_Manager()->log->writeToLog($e->getMessage());
		}
	}
	...
	public static function subscriptionWentActive(WebhookNotification $notification){
		...
		BT_Manager()->log->writeToLog(sprintf('Subscription %s is now active.', $subscription->id));
	}
	...
	public static function subscriptionPastDue(WebhookNotification $notification){
		...
		BT_Manager()->log->writeToLog(sprintf('Subscription %s is past due.', $subscription->id));
	}
	...
	public static function subscriptionExpired(WebhookNotification $notification){
		...
		BT_Manager()->log->writeToLog(sprintf('Subscription %s is expired.', $subscription->id));
	}
	...
	public static function processTestNotification(WebhookNotification $notification){
		BT_Manager()->log->writeToLog(sprintf(__('Webook test was successful. Payload received from Braintree at %s', 'braintree'), $notification->timestamp->format('m/d/Y H:i:s')));
