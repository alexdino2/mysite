...
class Braintree_Donations{
	...
	public static function getDonationScript(){
		if(self::$_isModal){
			Braintree_Donation_Scripts::getModalScript();
		}
		else{
			Braintree_Donation_Scripts::getInlineScript();
		}
	}
	...
	public static function processDonation(){
		...
		if(self::$_captureName){
			...
			$attrs['billing'] = array('firstName'=>$firstAndLast[0], 'lastName'=>$firstAndLast[1]);
			$attrs['customer'] = array('firstName'=>$firstAndLast[0], 'lastName'=>$firstAndLast[1]);
		}
		...
		if($response->success){
			wp_send_json(array('result'=>'success', 'url'=>self::getOption('donation_success_url')));
		}
		else{
			wp_send_json(array('result'=>'failure', 'message'=>$response->message));
		}
	}
	 ...
	 */
	public static function showDonationContainer(){
		ob_start();
		self::generateClientToken();
		self::getAjaxUrl();
		if(self::$_isModal){
			self::showModalDonationContainer();
		}
		else{
			self::showInlineDonationContainer();
		}
		$html = ob_get_clean();
		return $html;
	}
	 ...
	 */
	public static function showModalDonationContainer(){
		Braintree_Donation_Scripts::getModalScript();
		self::showModalButton();
		self::buildModalContainer();
	}
	
	/**
	 * Method that generates the inline html
	 */
	public static function showInlineDonationContainer(){
		Braintree_Donation_Scripts::getInlineScript();
	?>
	 ...
	 */
	public static function buildModalContainer(){
		?>
		<div class="overlay-payment-processing">
		  <?php self::getLoader()?>
		</div>
		<div id="donation_overlay" class="modal-overlay">
			<div class="modal-container">
			  <div id="donation_container">
			    <div class="errorMessages" id="error_messages"></div>
			    <div class="modal-button"><span id="cancel_donation"><?php echo __('Cancel', 'braintree')?></span></div>
				  <form name="donation-form" id="donation-form">
				  <?php if(self::$_captureName){
				    	echo self::buildInputField(array(
				    			'type'=>'text',
	...
	public static function generateClientToken(){
		?>
		<input type="hidden" name="client_token" id="client_token" value="<?php echo BT_Manager()->getClientToken(wp_get_current_user()->ID)?>"/>
		<?php 
	}
	
	public static function getAjaxUrl(){
		?>
		<input type="hidden" name="ajax_url" id="ajax_url" value="<?php echo admin_url().'admin-ajax.php?action=process_braintree_donation'?>"/>
		<?php
    }
