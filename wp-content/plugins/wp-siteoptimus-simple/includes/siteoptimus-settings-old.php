<?php
  ...
  $url = http_build_query( array(
                                'next'          =>  admin_url('admin.php?page=siteoptimus-settings'),
                                'scope'         =>  'https://www.googleapis.com/auth/analytics',
                                'response_type' =>  'code',
                                //'redirect_uri'  =>  'urn:ietf:wg:oauth:2.0:oob',
                                'redirect_uri'  =>  'http://ec2-52-87-149-12.compute-1.amazonaws.com/wp-admin/admin.php?page=siteoptimus-settings',
                                'client_id'     =>  '380690704201-a2noolvrg7g8ok1a4lf9cas17a45uvue.apps.googleusercontent.com'
                                )
                          );
  ...
  if ( isset( $_POST["save_code"]) and isset($_POST["access_code"]) ) {
    ...
    if( $wp_siteoptimus->wpa_save_data( $_POST["access_code"] )){
...
<div class="wrap">
  <h2 class='opt-title'><span id='icon-options-general' class='analytics-options'><img src="<?php echo plugins_url('wp-siteoptimus/images/wp-analytics-logo.png');?>" alt=""></span>
    <?php echo __( 'siteoptimus Plugin Settings', 'wp-siteoptimus'); ?>
  </h2>
  
  <?php
  if (isset($update_message)) echo $update_message;
  
  if ( isset ( $_GET['tab'] ) ) $wp_siteoptimus->pa_settings_tabs($_GET['tab']); 
  else $wp_siteoptimus->pa_settings_tabs( 'authentication' );
  ...
  <form action="<?php echo str_replace( '%7E', '~', $_SERVER['REQUEST_URI']); ?>" method="post" name="settings_form" id="settings_form">
    <table width="1004" class="form-table">
      ...
      <?php if( get_option( 'access_token' ) ) { ?>
        ...
        <tr>
          <th><?php esc_html_e( 'Clear Authentication', 'wp-siteoptimus' ); ?></th>
...
if( $tab == 'profile' ){
  $profiles = $wp_siteoptimus->pt_get_analytics_accounts();
  if( isset( $profiles ) ) { ?>
    <p><?php esc_html_e( 'Select profile for dashboard data.', 'wp-siteoptimus' ); ?></p>

    <form action="<?php echo str_replace( '%7E', '~', $_SERVER['REQUEST_URI']); ?>" method="post">
      <table width="1004" class="form-table">
        <tbody>
          <tr>
            <th width="115"><?php esc_html_e( 'Dashboard :', 'wp-siteoptimus' );?></th>
            <td width="877">
