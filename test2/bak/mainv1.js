function show() {
  ...
  // Load local data
  //var TOP_NODES_URL = TOP_NODES_FILENAME;
  //var TOP_EDGES_URL = TOP_EDGES_FILENAME;
  //var DETAILS_NODES_URL = DETAILS_NODES_FILENAME;
  //var DETAILS_EDGES_URL = DETAILS_EDGES_FILENAME;
  ...
  var topWeightScale = d3.scaleLinear().range([1, 4]);
  var detailsWeightScale = d3
    .scaleLinear()
    .domain([0, 1])
    .range([0, 4]);
  ...
  function renderTop() {
    ...
    link = topG
      ...
      .attr("stroke-width", function(d) {
        return topWeightScale(d.weight);
      });
    ...
    simulation.nodes(topNodes).on("tick", ticked);
    simulation.force("link").links(topEdges);
  ...
  function renderDetails(topNode) {
    ...
    if (nodesToRender.length === 0) {
      ...
      link = detailsG
        ...
        .attr("class", "link")
        .attr("stroke-width", function(d) {
          return detailsWeightScale(d.weight);
        });
      ...
      simulation.nodes(nodesToRender).on("tick", ticked);
      simulation.force("link").links(edgesToRender);
      simulation.alpha(1).restart();
    }
  }
